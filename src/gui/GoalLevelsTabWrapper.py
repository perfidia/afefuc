...
class GoalLevelsTableModel(QtCore.QAbstractTableModel):
	...
	def rowCount(self, index):
		return len(self.afefuc['project'].ucspec.goal_levels)
	...
	def data(self, index, role):
		...
		if column == 0 and role == QtCore.Qt.DisplayRole:
			return QtCore.QVariant(self.afefuc['project'].ucspec.goal_levels[index.row()].name)
	...
	def removeItem(self, position):
		...
		del(self.afefuc['project'].ucspec.goal_levels[position])
	...
	def updateItem(self, item):
		...
		for i, uc in enumerate(self.afefuc['project'].ucspec.goal_levels):
			if uc is item[0]:
				counter = i
				self.afefuc['project'].ucspec.goal_levels[i] = item[1]
				break
		else:
	...
	def insertItem(self, actor):
		...
		self.afefuc['project'].ucspec.goal_levels.append(actor[1])
	...
	def movePositionUp(self, position):
		...
		(
				self.afefuc['project'].ucspec.goal_levels[pos1],
				self.afefuc['project'].ucspec.goal_levels[pos2]
		) = (
				self.afefuc['project'].ucspec.goal_levels[pos2],
				self.afefuc['project'].ucspec.goal_levels[pos1]
		)
...
class GoalLevelsTabWrapper():
	...
	def show(self):
		...
		self.model = GoalLevelsTableModel(self.tab.itemsView, self.afefuc)
		...
		self.parent.mainWindow.tabWidget.addTab(self.can, _fromUtf8("Goal levels"))
