...
from gui.TermFormWrapper import TermFormWrapper
...
class GlossaryTableModel(QtCore.QAbstractTableModel):
	...
	def rowCount(self, index):
		return len(self.afefuc['project'].glossary)
	...
	def removeItem(self, position):
		...
		del(self.afefuc['project'].glossary[position])
	...
	def updateItem(self, item):
		...
		for i, uc in enumerate(self.afefuc['project'].glossary):
			if uc is item[0]:
				counter = i
				self.afefuc['project'].glossary[i] = item[1]
				break
		else:
			assert 1 == 2 and "item not found"
	...
	def insertItem(self, item):
		...
		self.afefuc['project'].glossary.append(item[1])
	...
	def movePositionUp(self, position):
		...
		(
				self.afefuc['project'].glossary[pos1],
				self.afefuc['project'].glossary[pos2]
		) = (
				self.afefuc['project'].glossary[pos2],
				self.afefuc['project'].glossary[pos1]
		)
